...
const filterOutOdds = (arr) => arr.filter(elem => elem % 2 === 0);

const filterState = (arr, state) => arr.filter(elem => elem.state === state);

const showOutOfCADevs = (arr) => arr.filter(elem => elem.state !== "CA").map(elem => elem.name.toUpperCase()).reduce((acc, elem) => `${acc}, ${elem}`);
